...
Реквизиты исполнителя (`EXECUTOR_*` в `backend/.env`) обязательны: с
`DEBUG = False` процесс не поднимется, пока пуст хотя бы один из пяти, и в
сообщении будут названы именно недостающие имена. Это сделано намеренно —
незаданный реквизит ничего не ломает, PDF собирается успешно, и пустой БИН
обнаруживается уже на бланке у заказчика.
...
# Реквизиты исполнителя — печатаются в договоре и счёте.
# Обязательны все пять, дефолтов нет: с DEBUG = False пустой или
# пропущенный реквизит валит запуск с перечислением недостающих имён,
# с DEBUG = True — предупреждение в stderr, старт не блокируется.
